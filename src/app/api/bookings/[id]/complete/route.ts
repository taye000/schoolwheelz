...
export async function POST(
  ...
  if (user.userType !== "driver")
    return NextResponse.json({ success: false, message: "Only drivers can end trips" }, { status: 403 });
  ...
  try {
    ...
    if (booking.status !== "in_progress")
      return NextResponse.json({ success: false, message: "Trip is not in progress" }, { status: 400 });
