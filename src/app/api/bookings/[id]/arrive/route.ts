...
export async function POST(
  ...
  if (user.userType !== "driver")
    return NextResponse.json({ success: false, message: "Only drivers can use this" }, { status: 403 });
  ...
  try {
    ...
    if (booking.status !== "accepted")
      return NextResponse.json({ success: false, message: "Booking must be accepted before arriving" }, { status: 400 });

    if (booking.arrivedAt)
      return NextResponse.json({ success: false, message: "Already marked as arrived" }, { status: 400 });

    booking.arrivedAt = new Date();
    await booking.save();

    const parent = await Parent.findById(booking.parent, "fullName phoneNumber");
